    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (!string.Equals(input, "quit", StringComparison.OrdinalIgnoreCase))
            {
                ...
                SudokuCell minimumPossibilitiesCell = _sudoku.MinimumPossibilities();
        ...
        private static void Fill()
        {
            ...
            foreach (SudokuCellCollection row in _sudoku.Rows)
            {
                foreach (SudokuCellCollection column in _sudoku.Columns)
                {                    
                    var inputInt = -1;
                    SudokuCell cell = _sudoku.Cells.Find(x => x.Row.Index == row.Index && x.Column.Index == column.Index);
        ...
        private static void Cell()
        {
            ...
            while (!string.Equals(input, "stop", StringComparison.OrdinalIgnoreCase))
            {
                ...
                while (!string.Equals(input, "stop", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    Console.WriteLine("Command\tDescription");
                    Console.WriteLine("'STOP'\tReturn to main menu.");
                    Console.WriteLine("'#'\tValid possibility. '0' to clear the cell.");
                    ...
                    if (inputInt == 0 || possibilities.Contains(inputInt))
                    {
                        cell.Value = inputInt;
                        return;
                    }
